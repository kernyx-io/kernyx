...
function ShipCard({ ship }: { ship: (typeof SHIPS)[0] }) {
  ...
  const broadsideCount =
    ship.heavyWeapons === "—" ? 0 : parseInt(ship.heavyWeapons.split("-")[1] ?? "0", 10);
  ...
  const filtered = useMemo(() => {
    return SHIPS
      ...
      .sort((a, b) => {
        ...
        if (sortBy === "broadside") {
          const ba = parseInt(a.heavyWeapons.split("-")[1] ?? "0", 10);
          const bb = parseInt(b.heavyWeapons.split("-")[1] ?? "0", 10);
          return bb - ba;
        }
        return 0;
      });
    ...
    <div className="ships-page">
      <div className="ships-header">
        ...
        <p className="ships-subtitle">
          Complete stats, roles, and tactical overview for every vessel in the Archipelago.
        </p>
      </div>
      ...
      <div className="ships-controls">
        ...
        <div className="ships-filter-group">
          <span className="ships-filter-label">Type</span>
          <div className="ships-chip-row">
            {TYPES.map((t) => (
              <button
                key={t}
                type="button"
                className={`ships-chip ${typeFilter === t ? "ships-chip-active" : ""}`}
        ...
        <div className="ships-filter-group">
          <span className="ships-filter-label">Rate</span>
          <div className="ships-chip-row">
            {RATES.map((r) => (
              <button
                key={r}
                type="button"
                className={`ships-chip ${rateFilter === r ? "ships-chip-active" : ""}`}
        ...
        <div className="ships-filter-group">
          <span className="ships-filter-label">Sort by</span>
          <div className="ships-chip-row">
            {(["rate", "durability", "speed", "broadside"] as const).map((s) => (
              <button
                key={s}
                type="button"
                className={`ships-chip ${sortBy === s ? "ships-chip-active" : ""}`}
                onClick={() => setSort(s)}
              >
