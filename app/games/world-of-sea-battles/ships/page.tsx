...
function ShipCard({ ship }: { ship: typeof SHIPS[0] }) {
  ...
  const broadsideCount = ship.heavyWeapons === '—' ? 0 : parseInt(ship.heavyWeapons.split('-')[1] ?? '0');

  return (
    <div className="ship-card">
      <div className="ship-card-header">
        <div className="ship-card-rate">{RATE_LABELS[ship.rate]}</div>
        <span className="ship-type-badge" style={{ color, borderColor: color }}>{ship.type}</span>
      </div>

      <div className="ship-card-name">{ship.name}</div>
      <p className="ship-card-info">{ship.info}</p>

      <div className="ship-stats">
        <StatBar label="Durability"    value={ship.durability}        max={4160} color={color} />
        <StatBar label="Speed"         value={ship.speed}             max={21}   color={color} />
        <StatBar label="Maneuver"      value={ship.maneuverability}   max={110}  color={color} />
        <StatBar label="Armor"         value={ship.broadsideArmor}    max={9}    color={color} />
        <StatBar label="Broadside"     value={broadsideCount}         max={63}   color={color} />
      </div>

      <div className="ship-footer">
        <div className="ship-footer-stat"><span>⚓</span><span>{ship.slots} slots</span></div>
        <div className="ship-footer-stat"><span>👥</span><span>{ship.crew} crew</span></div>
        <div className="ship-footer-stat"><span>📦</span><span>{ship.hold.toLocaleString()} hold</span></div>
        <div className="ship-footer-stat"><span>🔫</span><span>{ship.swivelGuns} swivels</span></div>
      </div>
    </div>
  );
}
...
export default function ShipsPage() {
  ...
  const filtered = useMemo(() => {
    return SHIPS
      ...
      .filter(s => rateFilter === 'All' || s.rate === parseInt(rateFilter))
      ...
      .sort((a, b) => {
        ...
        if (sortBy === 'broadside') {
          const ba = parseInt(a.heavyWeapons.split('-')[1] ?? '0');
          const bb = parseInt(b.heavyWeapons.split('-')[1] ?? '0');
          return bb - ba;
        }
        return 0;
      });
  ...
  return (
    <div className="ships-page">
      {/* Header */}
      <div className="ships-hero">
        <Link href="/games/world-of-sea-battles" className="ships-back">← Back to Hub</Link>
        <h1 className="ships-title">Ship Guide</h1>
        <p className="ships-subtitle">Complete stats, roles, and tactical overview for every vessel in the Archipelago</p>
      </div>
      ...
      <div className="ships-controls">
        ...
        <div className="ships-filters">
          ...
          <div className="filter-group">
            ...
            <div className="filter-pills">
              {RATES.map(r => (
                <button
                  key={r}
                  className={`filter-pill ${rateFilter === r ? 'active' : ''}`}
                  onClick={() => setRateFilter(r)}
                >{r === 'All' ? 'All' : `Rate ${r}`}</button>
              ))}
            </div>
          </div>
          ...
          <div className="filter-group">
            ...
            <div className="filter-pills">
              {(['rate', 'durability', 'speed', 'broadside'] as const).map(s => (
                <button
                  key={s}
      ...
      {/* Results count */}
      <div className="ships-count">
        Showing <strong>{filtered.length}</strong> of {SHIPS.length} ships
      </div>
